...
public value class Entropy(internal val entropy: ByteArray) {
    ...
    init {
        require(Strength.DEFAULT.size == entropy.size)
    }

    public fun toKeys(): Keys {
        val mnemonic = IntArray(Strength.DEFAULT.wordCount)
        (0 until mnemonic.size - 1).forEachIndexed { index, _ -> mnemonic[index] = extractWord(entropy, index) }
        mnemonic[mnemonic.size - 1] = extractLast(entropy, Strength.DEFAULT.wordCount - 1)

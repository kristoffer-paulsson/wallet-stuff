...
class Bip39Test {
    ...
    fun trezorJsonIter(file: String, passphrase: String, process: (lang: String, entropy: ByteArray, sentence: String, passphrase: String, p1: String /*, p2: String*/) -> Unit) {
        ...
        data.forEach {
            ...
            it.value.jsonArray.forEach { el ->
                ...
                process(lang, BinHex.decodeToBin(items[0]), items[1], passphrase, items[2] /*, items[3]*/)
            }
        }
    }

    fun bip32jpJsonIter(file: String, lang: String, process: (lang: String, entropy: ByteArray, sentence: String, passphrase: String, p1: String/*, p2: String*/) -> Unit) {
        val data = Json.decodeFromString<JsonArray>(file)
        data.forEach {
            val obj = it.jsonObject

            process(lang,
                BinHex.decodeToBin(obj["entropy"]!!.jsonPrimitive.content),
                obj["mnemonic"]!!.jsonPrimitive.content,
                obj["passphrase"]!!.jsonPrimitive.content,
                obj["seed"]!!.jsonPrimitive.content,
                /*obj["bip32_xprv"]!!.jsonPrimitive.content*/
            )
        }
    }

    @Test
    fun entropyToMnemonicTrezor() {
        trezorJsonIter(TREZOR.testVectors, "TREZOR") { l, e, m, p, p1/*, p2*/ ->
            val lang = language[l] ?: Language.ENGLISH
            ...
            assertContentEquals(bip39.mnemonic.toSentence(), m.normalize(Form.NFKD).encodeToByteArray())
            assertEquals(bip39.seed.toHex().lowercase(), p1.lowercase())
        }
    }

    @Test
    fun mnemonicToEntropyTrezor() {
        trezorJsonIter(TREZOR.testVectors, "TREZOR") { l, e, m, p, p1/*, p2*/ ->
            val bip39 = Bip39.fromSentence(m, language[l]!!, p)
    ...
    @Test
    fun entropyToMnemonicJp() {
        bip32jpJsonIter(BIP32JP_JP.testVectors, "japanese") { l, e, m, p, p1/*, p2*/ ->
            val bip39 = Bip39(entropy = Entropy(e), language = language[l]!!, passphrase = Passphrase(p),)
            assertContentEquals(bip39.mnemonic.toSentence(), m.normalize(Form.NFKD).encodeToByteArray())
            assertEquals(bip39.seed.toHex().lowercase(), p1.lowercase())
        }
    }
    ...
    @Test
    fun entropyToMnemonicEn() {
        bip32jpJsonIter(BIP32JP_EN.testVectors, "english") { l, e, m, p, p1/*, p2*/ ->
            val bip39 = Bip39(entropy = Entropy(e), language = language[l]!!, passphrase = Passphrase(p),)
            assertContentEquals(bip39.mnemonic.toSentence(), m.encodeToByteArray())
            assertEquals(bip39.seed.toHex().lowercase(), p1.lowercase())
        }
    }

    @Test
    fun mnemonicToEntropyEn() {
        bip32jpJsonIter(BIP32JP_EN.testVectors, "english") { l, e, m, p, p1/*, p2*/ ->
            val bip39 = Bip39.fromSentence(m, language[l]!!, p)
            assertContentEquals(bip39.entropy.entropy, e)
            assertEquals(bip39.seed.toHex().lowercase(), p1.lowercase())
    ...
    fun entropyMnemonicFuzzing() {
        ...
        language.forEach { l ->
            fuzzVectors.forEach { fv ->
                val e = Entropy(fv)
                val bip39A = Bip39(e, language = l.value)
